...
class Batcher {
  constructor({ keys, maxSize, batchPath, initialData = null }) {
  ...
  addObjectRelation(match) {
    ...
    if (!this.objectRelations.has(noun))
      this.objectRelations.set(noun, new Set([match]));
  ...
  addTenses() {
    for (let [key, value] of this.verbRelations) {
      let tenses = [];
      for (let v of value) {
        ...
        let conjugation = compromise(v).verbs().conjugate();
  ...
  stripRelations(relations) {
    ...
    for (let [key, values] of relations)
      data.set(key, new Set(Array.from(values)));
    return data;
  }
}

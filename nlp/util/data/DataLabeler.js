...
class DataLabeler {
  constructor(words, doLabel) {
    ...
    for (let i = 0; i < words.length; i++) this.verbRelations.set(words[i], new Set());
    if (doLabel) this.processLookups(words);
  }
  ...
  loadVerbs(words) {
    for (let i = 0; i < words.length; i++) {
      ...
      if (!lookup || !lookup.length) continue;
      for (let j = 0; j < lookup.length; j++) {
        const l = lookup[j];
        const pos = l.lexname.split('.')[0];
        const type = l.lexname.split('.')[1];
        if (words[i].pos != pos || !words[i].types.has(type)) continue; // if not the same part of speech skip it
        for (let k = 0; k < l.words.length; k++) {
          if (words[i].ignore.has(l.words[k])) continue;
  ...
  collectUses(batchSize, fromBeginning = false) {
    ...
    const trainingBatcher = new Batcher({
      keys,
      maxSize: batchSize,
      batchPath: './nlp/data/friends/training/',
      initialData: this.stripRelations(this.verbRelations),
    });
    const testBatcher = new Batcher({
      keys, maxSize: batchSize, batchPath: './nlp/data/friends/test/',
    });
    const verbRegex = Array.from(this.verbRelations.keys()).map(w => new VerbRegex({ word: w, synonyms: this.verbRelations.get(w), regExps: w.regex }));
    if (fromBeginning) this.fs.flushData('./nlp/data/friends/');
    const record = this.fs.readRecord('./nlp/data/friends/record.json');
    const start = { s1: 0, s2: 0 };
    ...
    for (let i = start.s1; i < friends.length; i++) { // for every episode
      ...
      for (let l = start.s2; l < lines.length; l++) {
        const line = lines[l];
        for (const r of verbRegex) {
          const matches = r.match(line);
          if (!matches.size) continue;
          for (const m of matches) {
            if (Math.random() <= 0.3) {
              testBatcher.addRelations({
                verb: r.word.text, match: m, i, l,
              });
            } else {
              trainingBatcher.addRelations({
                verb: r.word.text, match: m, i, l,
              });
            }
          }
  ...
  stripRelations(relations) {
    const data = new Map();
    for (const [key, values] of relations) data.set(key.text, new Set(Array.from(values)));
    return data;
  }

  printSnapshot(relations) {
    for (const [key, value] of relations) console.log(`${key.text} -> ${Array.from(value)}\n`);
  }
